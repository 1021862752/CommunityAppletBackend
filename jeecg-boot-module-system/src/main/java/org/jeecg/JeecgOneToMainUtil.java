...
public class JeecgOneToMainUtil {
	...
	public static void main(String[] args) {
		...
		mainTable.setTableName("md_base_order");//表名
		mainTable.setEntityName("BookingMainOrder");	 //实体名
		mainTable.setEntityPackage("shop");	 //包名
		mainTable.setFtlDescription("预约订单基本信息");	 //描述
		
		//第二步：设置子表集合配置
		List<SubTableVo> subTables = new ArrayList<SubTableVo>();
		//[1].子表一
		SubTableVo po = new SubTableVo();
		po.setTableName("md_booking_order");//表名
		po.setEntityName("BookingOrder");	    //实体名
		po.setEntityPackage("shop");	        //包名
		po.setFtlDescription("预约订单详情");       //描述
		//子表外键参数配置
		/*说明: 
